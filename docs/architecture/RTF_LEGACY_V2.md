...
6. **Independent release baseline:** final mathematical and performance claims
   compare the user-supplied original-only jar with SHA-256
   `A75ED34A2FA36F3222C75F193143A711589DF2E0078B14152FA582AF53E9D1E9`
   with the final modified jar in separate JVMs and separate runtime directories.
   Both runs use
...
- `Map` and `Blend` scalar calls retain the author's original bound-query order,
  branch short-circuits, and float expressions. `Map` computes stable bounds once
  per explicit batch fill only; it does not freeze unbound holder metadata at
  node construction.
...
- A bounded child-river traversal experiment was rejected. Its static subtree
  bounds were checked before recursive ancestor and descendant warps, lakes, and
  wetlands were applied, so legitimate deep children could be skipped. Legacy V2
  now uses the author's original `Network.Builder.build()`, `Rivermap`, and
  unconditional recursive traversal.
...
- The final NeoForge jar
  `31BA01F07EF6DB755DE40678DBBA89F0F45C3FA217189C1D96D799293ABA603F`
  was generated by a clean Gradle 8.8 build and compared in separate Java 25
  clients with the original-only jar. C2ME and OpenCL were disabled, audio used
  the null backend, and both clients used the same affinity, mod set, personal
  preset, seed, and fixed-spawn templates.
- Exact terrain-bearing chunk data matched at the river/wetland `(505,583)`,
  inland `(729,159)`, and archipelago/ocean `(1833,-1377)` samples. This
  includes block states, biomes, heightmaps, structures, post-processing,
  status, and terrain geometry. Full stable NBT also matched at the river and
  ocean samples; the inland comparison differed only in three scheduled
  `fluid_ticks`, while its blocks and heightmaps were identical.
- The crossed `original/final/final/original` client workload reduced mean
  start-region-to-player time from `15.586 s` to `12.819 s`, a 17.76% time
  reduction or 21.59% effective speed increase. Whole-process startup was not
  used for the release percentage because one original-jar trial stalled before
