...
public class PresetNoiseRouterData {
	...
	private static final double UNDERGROUND_COMPAT_DEPTH_MIN = 0.2D;
...
//        DensityFunction finalDensity = DensityFunctions.min(NoiseRouterData.postProcess(slideOverworld(slopedCheeseCaves, -worldDepth)), NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.NOODLE));
		...
		if(caves.densityAlgorithm == DensityAlgorithm.QUICK_V1) {
			...
			DensityFunction rangeInput = slopedCheese;
        ...
        return new NoiseRouter(aquiferBarrier, aquiferFluidLevelFloodedness, aquiferFluidLevelSpread, aquiferLava, temperature, vegetation, NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.CONTINENTS), NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.EROSION), climateDepth(caves, depth, y, worldDepth), NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.RIDGES), slideOverworld(DensityFunctions.add(initialDensity, DensityFunctions.constant(UNIT * -90)).clamp(-64.0, 64.0), -worldDepth), finalDensity, oreVeininess, oreVein, oreGap);
	}

    private static DensityFunction climateDepth(CaveSettings caves, DensityFunction depth, DensityFunction y, int worldDepth) {
        DensityFunction climateDepth = DensityFunctions.add(depth, DensityFunctions.constant(-0.205D));
        CompatibilityMode mode = caves.compatibilityMode;
        if(mode == CompatibilityMode.RTF || (mode == CompatibilityMode.AUTO && !TBCompat.isEnabled())) {
            return climateDepth;
        }
        DensityFunction terraBlenderDepth = climateDepth.clamp(-1.0D, terraBlenderClimateDepthMax(mode));
        if(mode == CompatibilityMode.AUTO && hasUndergroundBiomeCompatTarget()) {
            DensityFunction undergroundDepth = terraBlenderDepth.clamp(UNDERGROUND_COMPAT_DEPTH_MIN, terraBlenderClimateDepthMax(mode));
            return DensityFunctions.rangeChoice(y, -worldDepth, CAVE_BIOME_COMPAT_Y_MAX, undergroundDepth, terraBlenderDepth);
        }
        return terraBlenderDepth;
    }

    private static double terraBlenderClimateDepthMax(CompatibilityMode mode) {
        return mode == CompatibilityMode.AUTO && ModLoaderUtil.isLoaded("darkerdepths") ? 0.9D : 1.0D;
    }
